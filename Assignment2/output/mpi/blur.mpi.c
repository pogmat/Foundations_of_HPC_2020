	...
	if (pixel_size == 2) {
		...
		for (int i = 0; i < frame->writ.h; ++i)
			for (int j = 0; j < frame->writ.w; ++j) {
				...
				#ifdef VIGNETTING
				o_16[j + frame->writ.w * i] = (uint16_t)(((t0 + t1) + (t2 + t3)) / ((n0 + n1) + (n2 + n3)));
				#else
		...
		for (int i = 0; i < frame->writ.h; ++i)
			for (int j = 0; j < frame->writ.w; ++j) {
				...
				#ifdef VIGNETTING
				o_8[j + frame->writ.w * i] = (uint8_t)(((t0 + t1) + (t2 + t3)) / ((n0 + n1) + (n2 + n3)));
				#else
...
char* output_name(const char* infile, const char* k_type, const char* k_size, const char* k_param)
{
	...
	if (k_param) {
		...
		k_param_no_dot[j] = '\0';
		sprintf(out, "%s.b_%c_%sx%s_%s.mpi.pgm", infile_no_extension, k_type[0], k_size, k_size, k_param_no_dot);
	}
	else {
		sprintf(out, "%s.b_%c_%sx%s.mpi.pgm", infile_no_extension, k_type[0], k_size, k_size);
	}
...
int main(int argc, char** argv)
{
	...
	unsigned int kernel_size = (unsigned int)atoi(argv[2]);
        char* CLEANUP(cleanup_char) output_filename = NULL;
	char* input_filename;
	double kernel_param;
	...
	case 2:
		...
		output_filename = output_name(input_filename, argv[1], argv[2], NULL);
		if (argc == 5)
	...
	case 1:
		...
		input_filename = argv[4];
	 	output_filename = output_name(input_filename, argv[1], argv[2], argv[3]);     
		if (argc == 6)
