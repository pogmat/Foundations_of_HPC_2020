...
**********************************************************************************************/
  ...
  if(myid == 0)
	  printf("[MASTER]    Walltime: %10.8f\n", end_time - start_time);
  else
	  printf("[SLAVE %3d] Walltime: %10.8f\n", myid, end_time - start_time);

  //MPI_Barrier(MPI_COMM_WORLD);

  MPI_Reduce(&local_M, &M, 1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);

  MPI_Barrier(MPI_COMM_WORLD);

  if (myid == 0) {
	  pi = 4.0 * M / (N * numprocs);
	  printf("[MASTER] Number of trials: %llu, estimate of pi: %1.9f\n", N * numprocs, pi);
  }
